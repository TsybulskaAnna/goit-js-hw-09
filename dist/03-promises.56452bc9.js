...
var promises = document.querySelector(".form");
promises.addEventListener("submit", function (event) {
  event.preventDefault();
  var formElement = event.currentTarget.elements;
  var delay = Number(formElement.delay.value);
  var step = Number(formElement.step.value);
  var amount = Number(formElement.amount.value);

  for (var i = 1; i <= amount; i++) {
    createPromise(i, delay).then(function (_ref) {
      var i = _ref.i,
          delay = _ref.delay;

      _notiflixNotifyAio.Notify.success("\u2705 Fulfilled promise ".concat(i, " in ").concat(delay, "ms"));
    }).catch(function (_ref2) {
      var i = _ref2.i,
          delay = _ref2.delay;

      _notiflixNotifyAio.Notify.failure("\u274C Rejected promise ".concat(i, " in ").concat(delay, "ms"));
    });
    delay += step;
  }
});
